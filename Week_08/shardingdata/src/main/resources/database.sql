...
DROP PROCEDURE IF EXISTS ds0.create_shop_orders;

DELIMITER $$
$$
CREATE PROCEDURE ds0.create_shop_orders(IN IN_TIMES INT) 
BEGIN 
	DECLARE `@i` INT(11);
...
WHILE `@i`< IN_TIMES DO 
-- 创建表        
 SET	@createSql = CONCAT('CREATE TABLE IF NOT EXISTS shop_order', `@i`, "(`id` bigint UNSIGNED NOT NULL COMMENT '订单id',
  `sn` varchar(100)  NOT NULL COMMENT '订单编号',
  `order_status` varchar(25)  NOT NULL COMMENT '订单状态',
  `buyer_id` bigint UNSIGNED NOT NULL  NOT NULL COMMENT '买家id',
...
call ds0.create_shop_orders(16);


use ds1;

DROP PROCEDURE IF EXISTS ds1.create_shop_orders;

DELIMITER $$
$$
CREATE PROCEDURE ds1.create_shop_orders(IN IN_TIMES INT) 
BEGIN 
	DECLARE `@i` INT(11);
	DECLARE `@createSql` VARCHAR(2560);
	SET `@i` = 0;
...
call ds1.create_shop_orders(16);
